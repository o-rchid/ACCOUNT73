...
<html lang="en">
  <head>
    ...
    <script>
      ...
      function createSlip() {
        ...
        const slipGridOptions = {
          ...
          onRowClicked: function (event) {
            ...
            // console.log(selectedSlipRow.val());
            // console.log(JSON.parse(selectedSlipRow.val()).slipStatus);
            // console.log(event.data);
        ...
        if (
          ...
          if (confirmDelete()) {
            $.ajax({
              ...
              data: { slipNo: selectedSlipRow.slipNo },
              success: function () {
                ...
                // 그리드에서 선택된 전표 오브젝트를 못찾았다고 에러가 났는데 작동은 함.. 분개를 안넣어서 그런가?
                slipGrid.gridOptions.api.applyTransaction({
                  remove: [selectedSlipRow],
                }); //선택된 전표 삭제 (choi)
              },
            });
          }
        ...
        if (
          ...
          journalGrid.gridOptions.api.forEachNode(function (node, index) {
            if (node.data.journalNo !== "Total") {
              ...
              console.log(
                " JournalTotalObj.push(node.data) :" + JSON.stringify(node.data)
              );
              console.log(
                " JournalTotalObj.push(node.data) :" +
                  JSON.stringify(JournalTotalObj)
              );
            }
          });
      ...
      function createJournal() {
        ...
        const journalGridOptions = {
          ...
          getRowNodeId: function (data) {
            ...
            // use 'account' as the row ID
            console.log(data);
            console.log("getRowId: " + data.journalNo);
          ...
          onCellEditingStopped: function (event) {
            ...
            if (event.colDef.field === "accountCode") {
              ...
              $.ajax({
                ...
                data: { accountCode: event.data["accountCode"] }, // 계정코드,
                dataType: "json",
                async: false,
                success: function (jsonObj) {
      ...
      //             } else if (event.data["accountControlType"] == "TEXT") {
      //                 let str = prompt('상세내용을 입력해주세요', '');
      //                 gridOptions.api.applyTransaction([selectedJournalDetail["journalDescription"] = str]);
      //                 saveJournalDetailRow();
      //             } else {
      //                 gridOptions.api.applyTransaction([selectedJournalDetail["journalDescription"] = selectCal()]);
      //             }
      //
      //         },
      //         onCellValueChanged : function (event) {
      ...
      //
      //     };
      //     journalDetailGrid = new agGrid.Grid(document.querySelector('#journalDetailGrid'), gridOptions);
      // }
      ...
      function saveJournalDetailRow() {
        ...
        if (
          selectedJournalDetail["accountControlType"] == "SELECT" ||
          selectedJournalDetail["accountControlType"] == "SEARCH"
        )
          rjournalDescription =
            selectedJournalDetail[
              "journalDescriptionCode"
            ]; //- 숨겨진 곳에 저장한 값
        ...
        if (selectedSlipRow["slipNo"] !== "NEW") {
          ...
          $.ajax({
            ...
            success: function (jsonObj) {
              ...
              let detailGrid = journalGrid.gridOptions.api.getDetailGridInfo(
                "detail_" + event.value
              );
      ...
      function price(event) {
        ...
        if (
          totalRow.data["leftDebtorPrice"] != 0 &&
          totalRow.data["rightCreditsPrice"] != 0
        ) {
          if (
            totalRow.data["leftDebtorPrice"] !=
            totalRow.data["rightCreditsPrice"]
          ) {
      ...
      function createAccountDetailGrid() {
        ...
        const gridOptionsAccountDetail = {
          ...
          onCellDoubleClicked: function (event) {
            ...
            journalGrid.gridOptions.api.applyTransaction([
              (selectedJournalRow["accountCode"] =
                event.data["accountInnerCode"]),
            ]);
            journalGrid.gridOptions.api.applyTransaction([
            ...
            console.log(selectedJournalRow);
            console.log(
              "event.data[accountInnerCode] :" + event.data["accountInnerCode"]
            );
            $.ajax({
              //여기서부터는 분개상세
              type: "GET",
              ...
              success: function (jsonObj) {
                ...
                journalGrid.gridOptions.api.applyTransaction(
                  // [selectedJournalRow['journalDetailList'] = jsonObj['accountControl']]
                  [(selectedJournalRow["journalDetailList"] = jsonObj)]
                );
        ...
        accountDetailGrid = new agGrid.Grid(
          document.querySelector("#accountDetailGrid"),
          gridOptionsAccountDetail
        ); //div 태그에 붙임
      }
