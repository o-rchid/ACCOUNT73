        ...
        */
        ...
        function createDetailBudget() {
            ...
            gridOptions2 = {
                ...
                onRowClicked:function (event){   //행을 클릭시
                    ...
                    currentBudgetValueSet();   //당기 인풋값 초기화
                    currnetBudgetAppl();       // 당기 인풋에 값 넣기
                    previousBudgetValueSet();  // 전기 인풋값 초기화
                    previousBudgetAppl();      // 전기 인풋에 값 넣기
        ...
        function currentBudgetValueSet() {
            ...
            for(var a=1; a<=4; a++){   // 분기 금액 초기화 하기
                var sum=document.querySelector("#q"+a);
                sum.value=null;
            }
        ...
        function currnetBudgetAppl() {
            $.ajax({
                type: "GET",
                url: "${pageContext.request.contextPath}/budget/currentbudget",
                data: {
                    "budgetObj":JSON.stringify(dataSet),
                },
                dataType: "json",
        ...
        function inputCurrentBudgetAppl(data) {
            ...
            for(var i=1; i<=12;i++){
                var input=document.querySelector("#m"+i);
                input.value=numToMoney(data["m"+i+"Budget"]+"");
                if(input.value == "") num += 0;
                else num+=parseInt(input.value.split(",").join(""));//인풋의 밸류값이, 즉 3글자마다 잘린것에대해 숫자로 바꿈
                if(i%3==0){//i에 3을나눠서 0일떄 즉 3,6,9,12일시
                    var q=document.querySelector("#q"+i/3);//분기1,2,3,4
